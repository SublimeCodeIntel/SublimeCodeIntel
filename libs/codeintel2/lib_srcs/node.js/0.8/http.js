...
/**
 * Set the encoding for the request body. See [stream.setEncoding()][] for
 * more information.
 * @param encoding=null
 */
 ...
 * it's a [Buffer][]. Note that the data will be lost if there is no
 * listener when a ServerRequest emits a 'data' event.
...
/**
 * Set the encoding for the response body. See [stream.setEncoding()][] for
 * more information.
 * @param encoding=null
 */
 ...
 */
http.Server.prototype.listen = function(handle, listeningListener) {}
 ...
 * the tunneling stream, this may be empty. After this event is emitted,
 * the request's socket will not have a data event listener, meaning you
 * will need to bind to it in order to handle data sent to the server on
 * that socket.
 * @param request
 * @param socket
 * @param head
 ...
 * request body should be sent. See the ['checkContinue'][] event on
 * Server.
 */
 ...
 * how to listen for the upgrade event.
 * @param response {http.ClientResponse}
 * @param socket {net.Socket}
 * @param head {buffer.Buffer}
 */
